  ...
  switch (node.type) {
    ...
    case 'TryStatement':
      addChildren(node,
          ...
          'handler')
      markForDeletion(node, 'block', 'finalizer')
      break;
    case 'CatchClause':
      addChildren(node, wrap('Parameters', node.param), 'body')
      markForDeletion(node, 'param');
      break;
    case 'WhileStatement':
    ...
    case 'FunctionDeclaration':
      if (node.id !== null) {
        addChildren(node, wrap('FunctionIdentity', node.id))
        markForDeletion(node, 'id')
      }
      addChildren(node, wrap('Parameters', node.params), 'body')
      markForDeletion(node, 'params')
      break;
