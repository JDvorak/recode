...
function revert (node, parent) {
	...
	switch (node.type) {
		...
		case 'ThrowStatement':
			...
			break;
		case 'TryStatement':
			node.block = findOne(node, 'TryBlock', /^[A-z]*((Statement)|(Declaration))$/)
			node.finalizer = findOne(node, 'FinalizerBlock', /^[A-z]*((Statement)|(Declaration))$/)
			node.handler = findOne(node, 'CatchClause')
			break;
		case 'CatchClause':
			node.param = findOne(node, 'Parameters', /^[A-z]*((Expression)|(Identifier)|(Literal)|(MetaProperty))$/)
		...
		case 'ForStatement':
			...
			break;
		case 'ForInStatement':
			node.left = findOne(node, 'LeftOperand', /^[A-z]*((Expression)|(Identifier)|(Literal)|(Declaration)|(MetaProperty))$/)
		...
		case 'FunctionDeclaration':
			node.params = find(node, /Parameters/, /^[A-z]*((Pattern)|(Expression)|(Identifier)|(Literal)|(MetaProperty))$/ )
			node.body = findOne(node, /^[A-z]*BlockStatement$/ )
			node.id = findOne(node, 'FunctionIdentity', /^[A-z]*Identifier$/) || null 
			break;
...
function findOne(node, type, subtypes) {
